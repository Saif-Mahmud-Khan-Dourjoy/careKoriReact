...
export default function ModeratorModal({
  ...
  const isEdit = Boolean(initial?.role_id)

  const [avatarPreview, setAvatarPreview] = useState(null)

  useEffect(() => {
    if (initial?.moderator_profile?.avatar) {
      setAvatarPreview(initial?.moderator_profile?.avatar)
    } else {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      gender: initial?.moderator_profile?.gender || "male",
      district: initial?.moderator_profile?.district || "",
      sub_district: initial?.moderator_profile?.sub_district || "",
      dob: initial?.moderator_profile?.dob || "",
      avatar:  null,
      isEdit: isEdit,
    },
    validationSchema,
    onSubmit: async (values, { setSubmitting }) => {
      ...
      if (!isEdit) {
        formData.append("role", moderatorRole)
